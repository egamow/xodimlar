    ...
    <section class="content home">
        <div class="container">
            ...
            <div class="clearfix m-b-20">
                ...
                @endif
                <form action="{{ route('tb_violation.update', $tbViolation->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="id_tb">Қоидабузарлик тури</label>
                        <select class="form-control" name="id_tb">
                            @foreach($tbs as $tb)
                                <option @if($tbViolation->id_tb == $tb->id) selected
                                        @endif value="{{$tb->id}}">{{$tb->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control" name="user_id">
                            ...
                            @foreach($users as $user)
                                <option @if($tbViolation->user_id == $user->id) selected @endif
                    ...
                    <div class="form-group">
                        ...
                        <textarea type="text" class="form-control" placeholder="Изох"
                                  name="description">{{$tbViolation->description}}</textarea>
                    </div>
